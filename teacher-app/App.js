...
import {
  ...
  TouchableWithoutFeedback, StatusBar, Dimensions
} from 'react-native';
...
function CustomDrawer({ isOpen, onClose, onLogout, navigation, userName }) {
      ...
      <Animated.View style={[styles.drawer, { transform: [{ translateX: slideAnim }] }]}>
        ...
        <View style={styles.drawerHeader}>
          <View style={styles.drawerAvatar}>
            <Text style={styles.drawerAvatarText}>{userName ? userName[0].toUpperCase() : 'T'}</Text>
          </View>
          <Text style={styles.drawerName}>{userName || 'Teacher'}</Text>
          <Text style={styles.drawerRole}>Teacher Portal</Text>
        </View>
          ...
          screenOptions={({ route }) => ({
            tabBarIcon: ({ color, size }) => {
              const icons = {
                ...
                Marks: 'file-document', Homework: 'book-open-variant',
                Notices: 'bell', Profile: 'account'
              };
