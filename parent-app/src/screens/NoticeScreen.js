...
export default function NoticeScreen({ navigation }) {
  ...
  const fetchNotices = async () => {
    try {
      let response;
      try {
        response = await noticeAPI.getPublished();
      } catch {
        response = await noticeAPI.getAll();
      }
      const parentNotices = (response.data || []).filter(
        notice => notice.targetAudience === 'ALL' || notice.targetAudience === 'PARENTS'
      );
...
const styles = StyleSheet.create({
  ...
  noticeTitle: { fontSize: 18, fontWeight: 'bold', flex: 1, marginRight: 8, color: '#333' },
